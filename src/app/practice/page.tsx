...
const difficultyStyles: Record<Difficulty, string> = { Easy: "bg-brand-tint text-brand-darker", Moderate: "bg-amber-50 text-amber-700", Hard: "bg-red-50 text-red-600" };
...
export default function PracticePage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-[260px_1fr]">
      ...
      <main>{current ? <QuestionCard current={current} index={index} total={active.length} answers={answers} onAnswer={(answer) => setAnswers((value) => ({ ...value, [current.id]: answer }))} onPrevious={() => setIndex((value) => value - 1)} onNext={() => setIndex((value) => value + 1)} onSelect={setIndex} active={active} /> : <section className="rounded-2xl border border-dashed border-border bg-white p-12 text-center"><Circle className="mx-auto text-brand" /><h2 className="mt-4 font-display text-xl font-bold">No questions in this chapter yet</h2><p className="mt-2 text-sm text-muted">Your faculty can publish questions from the Admin Practice area.</p></section>}<button onClick={() => { setAnswers({}); setIndex(0); }} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-brand-darker"><RotateCcw size={15} /> Reset this chapter</button></main>
    </div>
  </div>;
}
