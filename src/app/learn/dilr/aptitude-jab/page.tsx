    ...
    <div className="flex flex-col gap-3">
      ...
      <div>
        ...
        <div className="mt-1.5 flex items-center gap-3">
          <span className={`rounded-full border px-2 py-0.5 text-[10.5px] font-semibold ${LOD_TEXT[video.lod]}`}>
            {LOD_LABEL[video.lod]}
          </span>
          {video.duration && (
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-[12.5px] font-semibold leading-snug ${isCompleted ? "line-through opacity-60" : "text-foreground"} ${isPlaying ? "text-brand-darker" : ""}`}>
          {item.title}
        </p>
        <p className="mt-0.5 text-[11px] text-muted">
          {isPlaying ? <span className="font-bold text-brand-dark">Now playing</span> : <>Set {item.set}</>}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className={`font-display text-[10px] font-bold ${isActive ? "text-brand-dark" : "text-muted"}`}>
          {String(index + 1).padStart(2, "0")}
        </span>
        <span className="min-w-0 flex-1 truncate text-[13px] font-semibold">{chapter.name}</span>
        {allDone && <CheckCircle2 size={14} className="shrink-0 text-brand" />}
      </div>
      <div className="mt-1.5 flex items-center gap-2 pl-5">
        ...
        <span className="text-[10px] font-medium text-muted">{doneCount}/{total}</span>
      </div>
    </button>
  );
}
...
function AptitudeJabPlayer() {
  ...
  const saveProgress = useCallback((next: Record<string, boolean>) => {
    ...
    saveTimer.current = setTimeout(() => {
      void setDoc(doc(db, "learn_progress", `${uid}_dilr`), { completed: next }, { merge: true });
    }, 800);
  }, [uid]);
  ...
  const activeChapter = filteredChapters[activeChapterIdx] ?? filteredChapters[0] ?? null;

  useEffect(() => {
    if (activeChapter) setActiveVideo(activeChapter.items[0] ?? null);
  }, [activeChapterIdx, activeChapter]);

  useEffect(() => { setActiveChapterIdx(0); }, [search]);
  ...
  const groupedVideos = useMemo(() => {
    ...
    return LOD_ORDER
      .map((lod) => ({ lod, items: activeChapter.items.filter((it) => it.lod === lod) }))
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-5 flex items-center gap-2 text-[13px] text-muted">
        ...
        <span>/</span>
        <Link href="/learn/dilr" className="hover:text-brand-darker font-medium">DILR</Link>
        <span>/</span>
        <span className="font-semibold text-foreground">Aptitude Jab</span>
      </div>
        ...
        >
          ...
          <div className="mb-3 rounded-xl bg-brand-tint/60 px-3 py-2.5">
            <div className="flex items-center justify-between">
              <p className="text-[11px] font-bold uppercase tracking-widest text-brand-dark">Your progress</p>
          ...
          <div className="relative mb-2">
            ...
            />
            {search && (
              <button type="button" onClick={() => setSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-full p-0.5 text-muted hover:text-foreground">
                <X size={12} />
              </button>
            )}
          </div>
          ...
          <div className="space-y-0.5">
            ...
            {filteredChapters.map((ch, idx) => (
              <ChapterRow
                ...
                doneCount={ch.items.filter((it) => completed[it.url]).length}
          ...
          {activeChapter && (
            <div className="glass-card-solid p-4 sm:p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <h2 className="font-display text-[15px] font-bold text-foreground">{activeChapter.name}</h2>
                  <p className="mt-0.5 text-[12px] text-muted">
                    {activeChapter.items.length} sets · {activeChapter.items.filter((it) => completed[it.url]).length} done
                  </p>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {LOD_ORDER.map((lod) => {
                    ...
                    return (
                      <span key={lod} className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${LOD_TEXT[lod]}`}>
                        {LOD_LABEL[lod]} · {count}
                      </span>
                    );
                  })}
                    ...
                    <div className="space-y-1">
                      {items.map((item) => (
                        <LessonRow
                          ...
                          onToggle={(e) => { e.stopPropagation(); toggleDone(item.url); }}
                        />
                      ))}
                    </div>
                  </div>
...
export default function AptitudeJabPage() {
  return (
    <Suspense
      fallback={
