  ...
  async function importBulk() {
    try {
      ...
      for (let start = 0; start < groups.length; start += 450) {
        const batch = writeBatch(db);
        groups.slice(start, start + 450).forEach((item) => batch.set(doc(collection(db, groupCollection)), { ...item, published: item.published !== false, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }));
        await batch.commit();
      }
