...
export function PracticeManager({ library = "practice" }: { library?: "practice" | "pyq" }) {
  ...
  async function saveManual() {
    try { validate(form); setSaving(true); const { error } = await createClient().from(questionTable).insert({ section: form.section, chapter: form.chapter.trim(), difficulty: form.difficulty, question: form.question.trim(), question_type: form.questionType || "MCQ", options: form.questionType === "TITA" ? [] : form.options.map((option) => option.trim()), correct_option: form.correctOption, correct_answer: form.correctAnswer || null, explanation: form.explanation?.trim() || "", published: form.published, position: Date.now() }); if (error) throw error; setForm(emptyQuestion()); setMessage(`${isPyq ? "PYQ" : "Practice"} question published for students.`); }
  ...
  async function importBulk() {
    try {
      ...
      setSaving(true);
      const questionRows = rows.map((item, index) => ({ section: item.section, chapter: item.chapter.trim(), difficulty: item.difficulty, question: item.question.trim(), question_type: item.questionType || "MCQ", options: item.questionType === "TITA" ? [] : (item.options || []).map((option) => option.trim()), correct_option: item.correctOption, correct_answer: item.correctAnswer || null, explanation: item.explanation?.trim() || "", published: item.published !== false, position: Date.now() + index }));
      ...
      if (questionRows.length) { const { error } = await createClient().from(questionTable).insert(questionRows); if (error) throw error; }
      if (groupRows.length) { const { error } = await createClient().from(groupTable).insert(groupRows); if (error) throw error; }
